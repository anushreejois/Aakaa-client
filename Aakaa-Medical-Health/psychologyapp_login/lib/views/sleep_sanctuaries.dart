...
class _SleepSanctuariesState extends State<SleepSanctuaries> {
  ...
  // Direct, stable nature ambient loops
  ...
  Future<void> _initPlayers() async {
    ...
    try {
      for (int i = 0; i < 4; i++) {
        // Load audio source with auto-looping enabled
        await _players[i].setAudioSource(
          AudioSource.uri(Uri.parse(_soundUrls[i])),
          preload: true,
        );
        await _players[i].setLoopMode(LoopMode.all);
        await _players[i].setVolume(_sounds[i]['volume']);
      }
      if (mounted) {
        setState(() {
          _isLoading = false;
      ...
      debugPrint("⚠️ Audio engine loading fallback active: $e");
      if (mounted) {
        setState(() {
          _isLoading = false;
        });
  ...
  @override
  void dispose() {
    // Release native media resources on screen pop
  ...
  Future<void> _updatePlayState() async {
    try {
      for (int i = 0; i < 4; i++) {
        final player = _players[i];
        final isEnabled = _sounds[i]['enabled'] && _isPlaying;
        if (isEnabled) {
          if (!player.playing) {
            await player.play();
          }
  ...
  void _toggleSound(int index) async {
  ...
  void _updateVolume(int index, double newVol) async {
    if (!PlanController.isSleepMixerAllowed && index > 0) return;
    setState(() {
      _sounds[index]['volume'] = newVol;
    });
    try {
      await _players[index].setVolume(newVol);
    } catch (e) {
